    ...
    link: function(scope, element, attrs,ngModel) {
      ...
      sortable.handleDrop = function(e) {
        ...
        if ( $window['drag_source'] != this) {
          ...
          if ( sortable.options &&  angular.isDefined(sortable.options.acceptance) ){
            $log.info('Acceptance function called');
              if(sortable.options.acceptance(ngModel.$modelValue,this.index,
              element.extra_data,$window['drag_source_extra'])){
                $log.info('Acceptance function passed');
                sort();
              }else{
                $log.info('Acceptance function failed');
                return;
              }
          }else{ // acceptanceFn not deffined
            sort();
          }
